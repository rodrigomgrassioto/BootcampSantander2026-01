...
public class BuilderGenerator {
    public TypeSpec create(String packageName, String className, String builderName, Map<String, TypeMirror> fields) {
        ...
        fields.keySet().forEach(f -> buildMethod.addStatement(
                "target.set$N($N)",
                f.substring(0,1).toUpperCase() + f.substring(1), // primera letra maiúscula
                f));

        buildMethod.addStatement("return target");

        return generatedBuilderClass
                .addMethod(buildMethod.build())
                .build();
    }

    private MethodSpec genBuilderSetter(
            String packageName, String name, String param, TypeName type){
